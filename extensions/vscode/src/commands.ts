...
function getFullScreenTab() {
  ...
  return tabs.find((tab) =>
    (tab.input as any)?.viewType?.endsWith("godcode.continueGUIView"),
  );
}
...
function focusGUI() {
  ...
  if (fullScreenTab) {
    ...
    // focus sidebar
    vscode.commands.executeCommand("godcode.continueGUIView.focus");
...
const getCommandsMap: (
  ...
  return {
    "godcode.acceptDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "godcode.rejectDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "godcode.defaultQuickAction": async (args: QuickEditShowParams) => {
      captureCommandTelemetry("defaultQuickAction");
      vscode.commands.executeCommand("godcode.focusEdit", args);
    },
    "godcode.customQuickActionSendToChat": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "godcode.customQuickActionStreamInlineEdit": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "godcode.codebaseForceReIndex": async () => {
      core.invoke("index/forceReIndex", undefined);
    },
    "godcode.rebuildCodebaseIndex": async () => {
      core.invoke("index/forceReIndex", { shouldClearIndexes: true });
    },
    "godcode.docsIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: false });
    },
    "godcode.docsReIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: true });
    },
    "godcode.focusContinueInput": async () => {
    ...
    "godcode.focusContinueInputWithoutClear": async () => {
      const isContinueInputFocused = await sidebar.webviewProtocol.request(
        "isContinueInputFocused",
        undefined,
        false,
    ...
    "godcode.writeDocstringForCode": async () => {
    ...
    "godcode.optimizeCode": async () => {
      captureCommandTelemetry("optimizeCode");
      streamInlineEdit("optimize", "Optimize this code");
    },
    "godcode.fixGrammar": async () => {
    ...
    "godcode.clearConsole": async () => {
      consoleView.clearLog();
    },
    "godcode.viewLogs": async () => {
      captureCommandTelemetry("viewLogs");
      vscode.commands.executeCommand("workbench.action.toggleDevTools");
    },
    "godcode.debugTerminal": async () => {
    ...
    "godcode.shareSession": async (sessionId: string | undefined) => {
      if (!sessionId) {
    ...
    "godcode.viewHistory": () => {
      vscode.commands.executeCommand("godcode.navigateTo", "/history", true);
    },
    "godcode.focusContinueSessionId": async (sessionId: string | undefined) => {
      if (!sessionId) {
    ...
    "godcode.applyCodeFromChat": () => {
      void sidebar.webviewProtocol.request("applyCodeFromChat", undefined);
    },
    "godcode.openConfigPage": () => {
      vscode.commands.executeCommand("godcode.navigateTo", "/config", false);
    },
    "godcode.selectFilesAsContext": async (
      firstUri: vscode.Uri,
      uris: vscode.Uri[],
    ) => {
      if (uris === undefined) {
    ...
    "godcode.forceAutocomplete": async () => {
    ...
    "godcode.openTabAutocompleteConfigMenu": async () => {
      ...
      quickPick.onDidAccept(() => {
        ...
        if (targetStatus !== undefined) {
          ...
          vscode.commands.executeCommand("godcode.focusContinueInput");
        } else if (selectedOption === "$(screen-full) Open full screen chat") {
          vscode.commands.executeCommand("godcode.openInNewWindow");
        } else if (selectedOption === "$(gear) Open settings") {
          vscode.commands.executeCommand("godcode.navigateTo", "/config");
        }
    ...
    "godcode.navigateTo": (path: string, toggle: boolean) => {
      sidebar.webviewProtocol?.request("navigateTo", { path, toggle });
      focusGUI();
    },
    "godcode.startLocalOllama": () => {
      startLocalOllama(ide);
    },
    "godcode.startLocalLemonade": () => {
      startLocalLemonade(ide);
    },
    "godcode.installModel": async (
      modelName: string,
    ...
    "godcode.convertConfigJsonToConfigYaml": async () => {
    ...
    "godcode.enterEnterpriseLicenseKey": async () => {
    ...
    "godcode.toggleNextEditEnabled": async () => {
    ...
    "godcode.openInNewWindow": async () => {
      ...
      vscode.commands.executeCommand("godcode.newSession");
      ...
      const sessionLoader = panel.onDidChangeViewState(() => {
        ...
        if (sessionId) {
          vscode.commands.executeCommand(
            "godcode.focusContinueSessionId",
            sessionId,
          );
        }
        panel.reveal();
